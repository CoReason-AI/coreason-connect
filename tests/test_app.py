...
def mock_transport():
    with patch("coreason_connect.app.SseServerTransport") as MockTransport:
        ...
        async def mock_handle_message_impl(scope, receive, send):
            await send({
                "type": "http.response.start",
                "status": 200,
...
@pytest.fixture
def client(mock_service, mock_config, mock_transport):
...
def test_sse_endpoint(client, mock_service, mock_transport):
...
def test_messages_endpoint(client, mock_transport):
